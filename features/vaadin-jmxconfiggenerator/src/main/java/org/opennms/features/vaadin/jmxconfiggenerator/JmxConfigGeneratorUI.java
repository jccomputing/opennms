        ...
        public void run() {
            try {
                ...
                try (JMXConnector connector = jmxConfigGenerator.getJmxConnector(config.getUser(), config.getPassword(), jmxServiceURL)) {
					final JmxDatacollectionConfig generateJmxConfigModel = jmxConfigGenerator.generateJmxConfigModel(connector.getMBeanServerConnection(), "anyservice", !config.isSkipDefaultVM(), config.isRunWritableMBeans(), Starter.loadInternalDictionary());
        ...
        private void handleError(final Exception ex) {
			UI.getCurrent().access(new Runnable() {
				...
				public void run() {
					...
					UIHelper.showNotification("Connection error", "An error occurred during connection. Please verify connection settings.<br/><br/>" + ex.getMessage(), Type.ERROR_MESSAGE);
					removeWindow(getProgressWindow());
				}
			});
        }
